...
def main():
    ...
    LUT_res = 37
    LUT = colour.LUT3D(name=f'Offset the negative values in BT.709 and compensate for luminance',
                       size=LUT_res)

    LUT.domain = ([[0, 0, 0], [1.0, 1.0, 1.0]])
    LUT.comments = [
        f'Lower Side of Guard Rail Formation for sRGB medium.',
    ...
    for i in range(x):
        for j in range(y):
            for k in range(z):
                ...
                # decode the input primaries from E-Gamut to Rec.709
                col = np.matmul(xyz_id65_to_bt709_i65 @ e_gamut_to_xyz_id65, col)
                ...
                col = np.matmul(xyz_id65_to_e_gamut @ bt709_id65_to_xyz_id65, col)
    ...
    colour.write_LUT(
        LUT,
        f"luminance_compensation_srgb.cube")
    print(LUT)
